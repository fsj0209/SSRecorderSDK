...
+ (void)addAudioPaths:(NSArray<NSString *> *)paths outputPath:(NSString *)outputPath callBack:(void (^)(void))handler {
    // 1. 获取音频源数组
    NSMutableArray<AVURLAsset *> *marrURLAsset = [NSMutableArray array];
    for (NSString *path in paths) {
        AVURLAsset *audioAsset = [AVURLAsset assetWithURL:[NSURL fileURLWithPath:path]];
        [marrURLAsset addObject:audioAsset];
    }
    // 2. 获取素材中的素材轨道数组
    NSMutableArray<AVAssetTrack *> *marrAssetTrack = [NSMutableArray array];
    for (AVURLAsset *asset in marrURLAsset) {
        AVAssetTrack *track = [asset tracksWithMediaType:AVMediaTypeAudio].firstObject;
        if (track) [marrAssetTrack addObject:track];
        NSLog(@"____track=%@", track);
    }
    // 3. 向音频合成器, 添加一个空的素材容器
    AVMutableComposition *composition = [AVMutableComposition composition];
    AVMutableCompositionTrack *audioTrack = [composition addMutableTrackWithMediaType:AVMediaTypeAudio preferredTrackID:0];
    ...
    for (NSInteger i = marrAssetTrack.count - 1; i >= 0; i--) {
        if (i != marrURLAsset.count - 1) audioAsset = marrURLAsset[i];
        [audioTrack insertTimeRange:CMTimeRangeMake(kCMTimeZero, marrURLAsset[i].duration) ofTrack:marrAssetTrack[i] atTime:(audioAsset ? audioAsset.duration : kCMTimeZero) error:nil];
    }
    // 5. 根据合成器, 创建一个导出对象, 并设置导出参数
    AVAssetExportSession *session = [[AVAssetExportSession alloc] initWithAsset:composition presetName:AVAssetExportPresetAppleM4A];
    session.outputURL = [NSURL fileURLWithPath:outputPath];
    session.outputFileType = AVAssetExportPresetAppleM4A;
    // 6. 开始导出数据
    __weak typeof(self) weakSelf = self;
    [session exportAsynchronouslyWithCompletionHandler:^{
        AVAssetExportSessionStatus status = session.status;
...
+ (void)convetCafToM4a:(NSString *)cafUrlStr
    ...
    [assetExport exportAsynchronouslyWithCompletionHandler:^{
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSInteger exportStatus = assetExport.status;
